...
3. `db.py` — SQLite CRUD; `get_connection()` auto-initializes schema on first call; three tables: `entries`, `invoices`, `active_session`
4. `timer.py` — start/stop/status using `active_session` table (enforces max 1 row)
